...
class Token {
public:
    enum Type { ID=0, LABEL, NUM, EOL, ERR, END, PUSH, JMPEQ, JMPGT, JMPGE, JMPLT,
        JMPLE, GOTO, SKIP, POP, DUP, SWAP, ADD, SUB, MUL, DIV, STORE, LOAD };
...
const char* Token::token_names[23] = { "ID", "LABEL", "NUM", "EOL", "ERR", "END",
                                       "PUSH", "JMEPEQ", "JMPGT", "JMPGE", "JMPLT", "JMPLE", "GOTO", "SKIP", "POP", "DUP",
                                       "SWAP", "ADD", "SUB", "MUL", "DIV", "STORE", "LOAD" };
...
Token* Scanner::nextToken() {
    ...
    char c;
    string lex;
    Token::Type ttype;//Inicio este token para capturar el lexema
    c = nextChar();
    while (c == ' ') c = nextChar();
